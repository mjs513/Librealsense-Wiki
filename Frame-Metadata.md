...
`librealsense2` package includes metadata patch for `uvcvideo` kernel object. The patches are intended to be deployed with Ubuntu 14 and Ubuntu 16.01 LTS with kernel version 4.4.0-xx., and are applied as part of Linux backend installation . See [[Linux installation-guide|installation-linux]] for more details.
...
Windows OS - the metadata is enabled if following requirements are met (see [[installation manual|installation-windows]] for details):
